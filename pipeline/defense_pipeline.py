# -*- coding: utf-8 -*-
"""Defense Procurement Intelligence - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["SAM.gov", "USASpending", "TED EU", "DoD 1260H"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Active Contracts", "value": "5,742", "delta": "+5%"},
            {"label": "Total Value", "value": "$89.2B", "delta": "+2"},
            {"label": "Contractors", "value": "1,203", "delta": "-3%"},
            {"label": "Countries", "value": "38", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "SAM.gov"}
            for i in range(1, 16)
        ],
        "timeseries": [
